...
int SecRilOpen()
{
    ...
    if (rilc->handle) {
        ...
        rilc->ril_set_audio_clock    = (int (*)(int))dlsym(rilc->handle, "SetCallClockSync");
...
int SecRilSetSoundClkMode(int mode)
{
    ...
    if(rilc) {
        if (rilc->ril_set_audio_clock) {
            if (mode)
                rilc->ril_set_audio_clock(VOICE_AUDIO_TURN_ON_I2S);
            else
                rilc->ril_set_audio_clock(VOICE_AUDIO_TURN_OFF_I2S);
        }
    }
